...
Ausgabe:
Die Ausgabe sind jsons beringten Tweets
...
def text_cleaner(text, remove_digits=True):
    ...
    string = re.sub(r'\.', '',string) 
    string = re.sub(r'\,', '',string) 
    # Rauschen welches sich in vorherigen Durchläufen herausstellte wird bereinigt
...
def clean(file_list):
    ...
    with open(os.getcwd() + r'\Datengewinnung\final_tweets_de_splitted\SARS_CoV.json'+'\\'+ file_list, 'r', encoding='utf-8') as f:
            tweets = json.load(f)
    ...
    with open(os.getcwd() + r"\Datengewinnung\final_tweets_de_clean\\SARS_CoV.json\\"+ re.sub(r'json',"",file_list), 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    directory = os.getcwd() + r'\Datengewinnung\final_tweets_de_splitted\SARS_CoV.json'
    path= os.getcwd() + r'\Datengewinnung\final_tweets_de_splitted\\SARS_CoV.json'
